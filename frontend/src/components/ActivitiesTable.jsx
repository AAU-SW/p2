...
export const ActivitiesTable = () => {
    ...
    async function handleSubmit(e) {
        ...
        try {
            const response = await axios.post("http://localhost:4000/activities", { title, price, date },
            {
                withCredentials: true
            });
            } catch(error) {
    ...
    return (
        <div>
            <button className="add-activity-button" onClick={() => setModal(true)}>+ Add new activity</button>
            ...
            <dialog open={modal} className={modal ? "backdrop" : ""}>
                <form className="inputForms" onSubmit={handleSubmit}>
                    <a className="form-header">Add new activity
                    ...
                    <button className="add-activities-button" type="submit" onClick={() => setModal(false)}>Submit</button>
                </form>
            </dialog>
        </div>
    );
